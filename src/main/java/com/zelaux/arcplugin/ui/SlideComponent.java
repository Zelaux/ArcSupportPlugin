...
public class SlideComponent extends JComponent{
    ...
    private SlideComponent.Unit myUnit = SlideComponent.Unit.LEVEL;
    ...
    enum Unit {
        ...
        private static float getMaxValue(SlideComponent.Unit unit) {
            return LEVEL.equals(unit) ? LEVEL_MAX_VALUE : PERCENT_MAX_VALUE;
        }

        private static @NlsSafe String formatValue(int value, SlideComponent.Unit unit) {
            return String.format("%d%s", (int) (getMaxValue(unit) / LEVEL_MAX_VALUE * value),
            unit.equals(PERCENT) ? "%" : "");
        }
    }

    void setUnits(SlideComponent.Unit unit) {
        myUnit = unit;
    }
    ...
    private void updateBalloonText() {
        ...
        myLabel.setText(myTitle + ": " + SlideComponent.Unit.formatValue(myValue, myUnit));
        if (myTooltipHint == null) {
